...
export class AccountsetupComponent implements OnInit {
  ...
  constructor(private router: Router,private toastr: ToastrService,private accountservice : AccoutsetupService,private authservice:AuthService) { }
  ...
  gotoApproveOrg() {
    ...
    if (!!this.password && !!this.confirmpassword && this.password.length >= 6) {
      if (this.password == this.confirmpassword) {
        this.accountservice.validatepassword(this.getOrgtoken, this.password, this.confirmpassword).then(resp => {
          ...
          if (!!this.userpassworddata.data) {
            ...
            this.onApprOrg=true;
            this.onApprUsr=false;
            this.accountservice.getAllorganisations().then(resp => {
  ...
  gotoApproveUsers() {
    this.accountservice.approveOrganisation().then(resp => {
      ...
      if (this.approveOrgdata == "yes") {
        ...
        this.accountservice.getPendingUsers().then(resp => {
      ...
      if (this.approveOrgdata == "no") {
        ...
        this.onApprGrp=true;
        this.accountservice.getAllpendingGroup().then(resp => {
  ...
  rejectOrgData() {
    this.accountservice.rejectOrganisation(this.comment).then(resp => {
  ...
  approvePendingUser() {
    ...
    if(this.checkedValueArray.length != 0){
      ...
      this.accountservice.approveUser(this.checkedValueArray).then(resp => {
        ...
        // this.storage.setData("chlogin_data", this.getToken);
         ...
         //this.authservice.saveToken(this.getToken);
...
this.displayTncmodal='block'
  ...
  gotodashboard(){
   ...
   rejectUser() {
    this.accountservice.rejectUser().then(resp => {
  ...
  approvePendingGroup() {
    ...
    if(this.checkedValueArray.length != 0){
      this.accountservice.approveGroup(this.checkedValueArray).then(resp => {
        ...
        if (this.approveGroupdata == "succes") {
          ...
          this.onApprGrp=false;
          this.accountservice.getAllPendingRule().then(resp => {
  ...
  approvependingRule() {
    ...
    if(this.checkedValueArray.length != 0){
      this.accountservice.approveRule(this.checkedValueArray).then(resp => {
...
//removable code
  ...
  rejectPendingGroup() {
    this.accountservice.rejectGroup(this.comment).then(resp => {
  ...
  getUserEmail(){
    ...
    // })
    this.accountservice.getUserDetails().then(res=>{
      //console.log(res['Data']['email'])
      this.useremail=res['Data']['email'];
    })
  }
