...
@Component({
  selector: 'app-pending-bill-payments',
  templateUrl: './pending-bill-payments.component.html',
  styleUrls: ['./pending-bill-payments.component.css']
})
export class PendingBillPaymentsComponent implements OnInit {
...
this.loaderService.display(true)
  var rejectdata={
    "invoiceIds":this.checkedValueArray,
    "regcmt":this.rejectcomnt,
  }
  this.paymentservice.rejectpaymentsNew(rejectdata).then(resp=>{
    if(resp['msg']=='Succes'){
      this.loaderService.display(false)
      this.router.navigate(['/main/rejectmsg'],{queryParams:{msg:'paynewreject'}});
    }else{
      this.toastr.warning("Failed to reject payment!","Alert",{
        timeOut:3000,
...
private loadPayments(){
  this.loaderService.display(true)
  this.paymentservice.getPendingPaymentsNew().then(resp=>{
    console.log(resp)
    this.paymentData=resp['data']['invcdata'];
    this.loaderService.display(false)
...
private getUserDetail(){
  this.userservice.getUserDetails().subscribe(res=>{
   ...
   // this.username=this.userdata['firstname']+" "+this.userdata['lastname']
    ...
    gotoOTP(): void {
          if (this.temp == true) {
            this.router.navigate(['/main/otp-approve-paymentnew', JSON.stringify(this.checkedValueArray)]);
          }else {
