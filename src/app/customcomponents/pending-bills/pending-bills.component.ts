...
this.displayreason='';
 ...
 approveBtn(){
  ...
  this.router.navigate(['main/otp-approve-billernew']);

}
rejectBtn(){
this.loaderService.display(true);
  var rejectdata={
    "vendorIds":this.checkedValueArray,
    "regcmt":this.rejectcomnt,
   
  }

  
  this.billservice.rejectbillsNew(rejectdata).then(resp=>{
    if(resp['Msg']=='Biller Rejected '){
      this.loaderService.display(false)
      this.router.navigate(['/main/rejectmsg'],{queryParams:{msg:'billnewreject'}});
    }else{
      this.loaderService.display(false)
    ...
    console.log(error)
    this.loaderService.display(false)
      this.toastr.error("Failed to reject!!","Alert",{
        timeOut:3000,
        positionClass:'toast-top-center'
        })
  ...
  private loadPendingbills(){
    this.loaderService.display(true)
  this.billservice.getPendingbillersNew().then(resp=>{
    console.log(JSON.stringify( resp['bills']));
    this.pendingbillers=resp['data']['billerdata'];
    if(this.pendingbillers==null){
      this.billerlength=0
      this.noofrole="No bills available"
      this.loaderService.display(false)
  ...
  gotoOTP(): void {
    if (this.temp == true) {
      ...
      this.router.navigate(['/main/otp-approve-billernew', JSON.stringify(this.checkedValueArray)]);
    } else {
