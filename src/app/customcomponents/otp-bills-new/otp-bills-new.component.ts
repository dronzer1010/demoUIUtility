...
@Component({
  selector: 'app-otp-bills-new',
  templateUrl: './otp-bills-new.component.html',
  styleUrls: ['./otp-bills-new.component.css']
})
export class OtpBillsNewComponent implements OnInit {
  validateOPT: any = [];
  ...
  verifyOtp(){
    ...
    if (checkOtp.length == 5) {
      var checkOtp = this.txt1 + this.txt2 + this.txt3 + this.txt4 + this.txt5;
      this.billservice.validateOTPNew(checkOtp).then(resp => {
        this.approveBills = resp;
        if (!!this.approveBills.Msg && this.approveBills.Msg == "Vendor approved ") {
          this.loaderService.display(false)
